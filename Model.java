...
public class Model extends Observable {
     ...
     */
        ...
        if(side==Side.SOUTH){
            _board.setViewingPerspective(Side.SOUTH);
            for (int col = 0; col < _board.size(); col += 1){
                movenorth(col);
            }
        ...
        if(side==Side.EAST){
            _board.setViewingPerspective(Side.EAST);
            for (int col = 0; col < _board.size(); col += 1){
                movenorth(col);
            }
            _board.setViewingPerspective(Side.NORTH);
    ...
    // 检查t上面的那块的值是不是和t一样
    public boolean checkupsame(int col,int row){
        if(_board.tile(col, row + 1)!=null) {
            if (_board.tile(col, row).value() == _board.tile(col, row + 1).value())
                return true;
    ...
    //检查t上面那块的值是不是空的
    public boolean checkupnull(int col,int row){
        if(_board.tile(col,row+1)==null)
            return true;
        else
            return false;
    }
    public boolean checkifend(int col){
        for(int row=_board.size()-2;row>=0;row -= 1){
            if(_board.tile(col,row)!=null) {
                if (checkupsame(col, row)) {
                    return false;
                }
                if (checkupnull(col, row)) {
                    return false;
                }
            }
        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        return false;
            ...
            public static boolean adject(Board b){
                for(int row=0;row<b.size();row +=1) {
                    for (int col = 0; col < b.size(); col += 1) {
